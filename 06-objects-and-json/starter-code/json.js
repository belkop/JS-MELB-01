...
// Copy the entire object above ({...}, but NOT 'var notActuallyJSON =' or the
// trailing semicolon)--just copy everything between the brackets, including the
// brackets themselves.
// Then go to http://jsonlint.com/ and paste it into the validator. Does it pass
//the test of being valid JSON?

// The following line is valid JavaScript, but the object is not written in
// valid JSON syntax.
// [ Step 2 ] Use JSONLint to validate the object in the following line; then,
// edit the object so it passes the validator.
var eddie = {name:"Eddie Vedder", age:49};

// We frequently want to turn a JavaScript object into a JSON string, and vice
// versa.
// JavaScript has a built-in global object called `JSON` that contains a number
// of useful methods for manipulating JSON.
// To turn a JS Object into JSON, use the .stringify method  ( JSON.stringify(someObject) )
// (See https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/JSON/stringify)

// [ Step 3 ] Turn the `grungeAlbums` JavaScript object into a JSON string, and
// store it in `grungeAlbumsJSON`.
// Then, convert `grungeAlbumsJSON` back into a JavaScript object, and compare it
// to the original `grungeAlbums`. Are they the same?
...
// Imagine that we only had JSON to work with, and wanted to convert it to a
// JS Object.
...
// Then, for each album in the list, do a console log printing out the album name,
// artist, and units sold.
...
// Convert it back to a JavaScript object, change it, and then convert it back
// to JSON again. Compare it to your original - how has it changed?
