...
## Object Properties
An object is an associative array, also known as a hash or a dictionary in other languages. It stores key-value pairs, and unlike arrays, is not ordered. Object properties are like variables attached to a specific object.
...
```js
  for (var key in myHouse) {
    if (myHouse.hasOwnProperty(key)) { // The hasOwnProperty method returns true if the nominated key exists on the object.
        console.log(key + " = " + myHouse[key] + "\n");
    }
  }
```

Copying an object into another one does not duplicate the object, but rather references it - ie points to the exact same location in memory.
...
We start by identifying the nouns and noun phrases: some of those nouns will
become objects. There are also implicit nouns here: even though it's not mentioned, there's likely to be a shopping cart object.
...
>*Note: An object is a thing, represented by a set of data -- its attributes -- that has functions associated with it to perform certain actions -- its methods. We have so far worked with objects as associative arrays, however we haven't used some of the superpowers we are given for JavaScript objects such as methods (object functions) and the prototype.*

An associative array is one in which we use keys to refer to each element, rather than element numbers. For example:
  address['street']
  address['postcode']
rather than:
  address[0]
...
Whilst the items in address can be accessed like an array, it is not an array - its an object - element numbers are not relevant.

#### Try it for yourself!

Consider one or two of the following scenarios.  Identify likely objects, attributes, and methods in each scenario.  Remember to consider implicit objects as well as explicit ones.
...
- A user who had reserved a rental car arrives to find it has not been returned yet, and customer service transfers her reservation to an available car.

- A computer game allows the user to take the role of a unit commander or general in a WWII battle and simulates the battle based on his or her commands.
...
**The object**
We're familiar with the new Object() syntax from our first example today. We create an instance of our "Person" object in a similar way.
...
**The constructor** is called at the moment that our new object is instantiated. The constructor is most often used to set the object's properties or to call methods to initialise the object.
...
While working with objects, we will run into the keyword **this** quite often. We will cover **this** in much more detail later on in the unit, however it's important to understand in the context of our objects that **this** refers to the current object instance.
...
```js
  var Superhero = function (firstName, superheroName) {
    ...
    // are set through the 'this' keyword with the values passed into the constructor function as parameters.
...
**Methods** are functions contained in our objects. We can call our object methods the same way we access our object properties - through the dot notation - with the main difference being that we add () at the end of our statement, just like any
other function all. To define a method, we assign a function to a named property of the object. However, we need to do this a certain way - using the objects prototype.
...
* eatSomething: accepts a string parameter for what to eat
...
Follow instructions in the [objects.js](starter-code/objects.js) file to complete the challenge!
...
Follow instructions in the [json.js](starter-code/json.js) file.
