...
>Last class, we worked on using basic commands in the terminal, writing JavaScript, and working with Git and GitHub.
...
| Objects | A group of properties and methods | {name: 'fred', getName: function() {...}} |
...
Javascript can tell us what data type a value is, using the builtin function 'typeof()'.

[`typeof()`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/typeof). Let's try it out in the terminal with the following:
...
You can also see if the value is of a specific type:
...
Notice the ===. Thats a comparison operator that means 'exactly equal to'. Its similar to '==', but '==' involves type coercion, and thats a bad thing. Never use '==', but rather always use '===' to compare two values.

`typeof()` returns a string with the type of the value you're looking at.

  >Note: At this point we haven't explained exactly what objects are. Objects are a collection of properties, and a property is an association between a key and a value. Objects in JavaScript are used in two ways:
...
Numbers are typically divided into two groups:
...
We use operators to work with data in JavaScript. The standard [arithmetic operators](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators#Arithmetic_operators) — which you have been learning since primary school — are supported here, including addition, subtraction, division, and so forth. Check it out:
...
JavaScript is slightly limited regarding the number of operations it allows you to perform. For example, it doesn’t enable you to easily square a number or cube a number. Fortunately, a special `Math` object with some very useful methods is available. The Math object is like a built in library or toolkit. It contains a set of functions and values for working with numbers.
...
Variables are used to store data types in a computer’s memory, so that they can be referenced later. They maintain the state of your program.
...
If you declare a variable without assigning a value to it, both its type and value will be `undefined.`
...
Let’s try assigning a value to a variable:
...
Always remember that these variables should always have the `var` keyword and the convention is to use `camelCase` when naming variables.

If you create a variable without using the var keyword, then the variable exists in the global scope. We will examine scope in a future lesson.
...
* If you want to turn a number into a string, you can use a helpful method called `toString` that exists on the data type objects (Number, String, Boolean, Date, Object).
...
//much clearer ti use the data type objects
Number(1).toString();
=> "1"
```
...
You will find that individual strings and numbers are often not enough for most programming purposes. What is needed are collections of data that we can use efficiently. These are called Arrays.
...
In this exercise, you will obtain and set array indices, as well as continue practicing with string concatenation. You will be populating two arrays: one with strings, one with numbers. You will then return these strings and numbers from the arrays in order to create sentences.
...
Note: There are some subtle differences between using the new keyword and using an array literal to create an array:
...
Using the push() method will an a new element to the end of the array.
Using the literal assignment allows you to add or update any element in the array.
...
In this exercise, you will utilize your knowledge of array helper methods in order to decode a secret message.
...
Note: The `.reverse()` method mutates the array structure - ei each element in the array is changed to reflect the reversal.
...
The 'for' loop is not specific to arrays - what you do within the loop is entirely up to you.
- The first argument of the loop is the iterator variable - you can set it to whatever value you like.
- The second argument is the condition - the loop will keep going while-ever this condition is true.
- The third argument indicates what happens at the end of each cycle.
