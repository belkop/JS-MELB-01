...
Each of these methods returns one or more elements from the DOM. What can we do with an [Element](https://developer.mozilla.org/en-US/docs/Web/API/Element)?
...
The above `window.onload` function adds a new element to our page through the following steps:
  ...
  4. The H1 element is added to the body. Both steps 3 and 4 use the `appendChild` method to the respective element.
...
You'll add the ability to complete tasks in the list. Your program must implement the following requirements:
- Change the value of the element with the id of 'name' to be whatever the user inputs when page first loads.
...
Short and powerful! Wouldn't it be nice if we could select elements in the same way?
...
Reading the documentation, we discover that `.val()` is a getter on an element, but that `.val(value)` is a setter on an element. Be sure you're using the correct method. Reading examples is very helpful, and the jQuery examples in the docs are fully functional!
...
Targeting DOM elements in jQuery is a little easier to read than the native DOM functions.
